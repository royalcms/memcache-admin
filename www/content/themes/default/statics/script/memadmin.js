    ...
    app.admin_server = {
        execute_command: function () {
            $('.execute_command').on('click', function () {
                ...
                $("#frist-content").hide();
                var $this = $(this);
                var value = $this.val('title');
                var url = $(this).attr('data-url');
                if (value != '') {
                    var postdata = {
                        request_command: $('#request_command').val(),
                        request_key: $this.attr('title'),
...
function changeServer(obj) {
    window.location = 'index.php?m=memadmin&c=server&a=init' + obj.options[obj.selectedIndex].value;
}

function changeServerSlab(obj) {
    window.location = 'index.php?m=memadmin&c=server&a=slabs' + obj.options[obj.selectedIndex].value;
}

function changeCluster(obj) {
    if (obj.options[obj.selectedIndex].value != '') {
        window.location = 'index.php?m=memadmin&c=monitor&a=init&cluster='
            + obj.options[obj.selectedIndex].value;
    } else {
        window.location = 'index.php?m=memadmin&c=monitor&a=init';
    }
}
...
function executeTelnet(target) {
    if (document.getElementById('request_telnet').value != '') {
        var request_url = 'index.php?m=memadmin&c=admin_access&a=init'
            + '&request_telnet='
...
function addCluster() {
    ...
    clusterDiv.innerHTML = '<hr/><strong>Cluster <input type="text" style="width:200px;" name="cluster[' + cluster_id + ']" value=""/></strong>'
        ...
        + '<div style="width:150px;float:left;">名称（选项）</div>'
        + '<div style="width:150px;float:left;margin-left:7px;">IP地址/主机名</div>'
        + '<div style="width:50px;float:left;margin-left:7px;">端口</div></div>'
        + '<div style="margin-left:40px;margin-top:6px;" id="cluster_' + cluster_id + '_commands">'
        + '<a class="btn btn-gebo list butto memcache" href="javascript:addServer(' + cluster_id + ')">'
        + '将新服务器添加到集群</a> <a class="btn btn-gebo list memcache" style="padding:1px 2px;-moz-border-radius:3px;-webkit-border-radius:3px;" '
        + 'href="#" onclick="deleteServerOrCluster(\'cluster_' + cluster_id + '\')">删除集群</a></div><br/>';
...
function addServer(current_cluster_id) {
    ...
    server_id++;
    serverDiv.innerHTML = '<br>'
        + '<div style="margin-left:30px;margin-top:3px;">'
        + '<input type="text" style="width:240px;" name="server[' + current_cluster_id + '][' + server_id + '][name]"'
        + '		  value="" id="name_' + server_id + '" onchange="nameOnChange(' + server_id + ') onKeyUp="hostOrPortOnChange(' + server_id + ')""/> '
        + '<input type="text" style="width:240px;" name="server[' + current_cluster_id + '][' + server_id + '][hostname]"'
        + '       value="主机名" id="host_' + server_id + '" onfocus="hostOnFocus(this)" onblur="hostOnBlur(this)" onchange="hostOrPortOnChange(' + server_id + ')"'
        + '       onchange="hostOrPortOnChange(' + server_id + ')" onKeyUp="hostOrPortOnChange(' + server_id + ')" /> '
        + '<input type="text" style="width:80px;" name="server[' + current_cluster_id + '][' + server_id + '][port]"'
        + '       value="端口" id="port_' + server_id + '" onfocus="portOnFocus(this)" onblur="portOnBlur(this) onchange="hostOrPortOnChange(' + server_id + ')"/> '
        + '<a class="btn list button memcache" style="padding:1px 2px;" href="#" onclick="deleteServerOrCluster(\'server_' + server_id + '\')">删除</a>' + '</div>';
